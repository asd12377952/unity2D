...
public class _001guy : MonoBehaviour {
	...
	void Update () {
		...
		if(distance < 15 && ydistance < 8){
			...
			if(distance > 10 && distance < 20 && !shooting){
				if(target.transform.position.x < transform.position.x){
					vel.x = -runSpeed;
				}
				if(target.transform.position.x > transform.position.x){
	...
	public IEnumerator shootBullet () {
		...
		if(bulletPosition.position.x < transform.position.x){
			...
			bullet = Instantiate(enemyBullet1, bulletPosition.position, Quaternion.Euler(0,0,0)) as GameObject;
		}
		//發射完後也要有空檔準備再次觸發射擊條件
		yield return new WaitForSeconds(0.2f);
		shooting = false;
